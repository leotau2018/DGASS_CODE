function [x, lambda, value, num_AS] =  DASA_SCREEN_7(A, y, l, u, lambda0, varargin)
...
%         lambdak_old = lambdak_raw(~AS_0); 
    ...
    if k >1
        ...
        if (Newton > 0) && (r<1e-2)%&& (screening == 1)
%             inner_I_AtA = (alphak*eye(sum(J_x_indi)) + A(:,J_x_indi)' * A(:,J_x_indi));
%             Bk_inv_fun = @(lambda) (lambda - A(:,J_x_indi)*(inner_I_AtA\(A(:,J_x_indi)'*lambda)));
%             Bk_inv_Axk_old = Bk_inv_fun(Axk_old);
            Bk = A_active * A_active';
%             Bk = eye(size(A,1)) + A_active * A_active';
            Bk_inv_Axk_old = Bk\Axk_old;
        else
            l_Axk_old = l - Axk_old;            % h_l_old is also the (sub)gradient corresponding to AS_p
            u_Axk_old = u - Axk_old;            % h_u_old is also the (sub)gradient corresponding to AS_n
        end
        ...
        while 1  % nonmonotone linear search for sparsa / proximal gradient algorithm (PGA)
            if (Newton > 0) && (r<1e-2)%&& (screening == 1)
                [lambdak_l, lambdak_u] = Newton_type(lambdak_old, Axk_old, l_Axk_old, u_Axk_old, Bk, alphak, OPTION);
                lambdak_l = lambdak_old - Bk_inv_Axk_old/alphak + l/alphak;
                lambdak_u = lambdak_old - Bk_inv_Axk_old/alphak + u/alphak;
            else
...
%                     lambdak = Trunc(lambdak, AS_p(~AS_0), AS_n(~AS_0)) ;
    ...
    %=vvv============== second line search for the real step size, seems NOT Necessary
...
%         end
    ...
    r = sqrt( 2*(pvalue_scaler - dvalue)/gamma1 );
    % only when the detection is safe(r is sufficient small -> the ragion is sufficient narrow)
    if  active_method == 1
...
%% compute Newton type direction
function [lambdak] = Newton_type(lambdak_old, Axk_old, l_Axk_old, u_Axk_old, Bk, alphak, OPTION)
% maxiter = OPTION.Newton.maxiter;
% tol = OPTION.Newton.maxiter;
maxiter = 10;
tol = 1e-3;
sigma = 0.2;
bar_lambda = lambdak_old;
Bk_hat = Bk - eye(size(Bk))/alphak;

f_bar_gradient = @(lambda) norm(-Axk_old + Bk_hat*(lambda - lambdak_old));
f_bar_value = @(lambda) (f_bar_gradient)'* (lambda - lambdak_old);
for i = 1: maxiter
    f_bar_gradient_l = f_bar_gradient(bar_lambda)+l;
    f_bar_gradient_u = f_bar_gradient(bar_lambda)+u;
    if norm(f_bar_gradient_l(f_bar_gradient_l<0), "inf") + norm(f_bar_gradient_u(f_bar_gradient_i<0), "inf") < tol
        break
    end
    eta_s = 1;
    s = 0;
    while 1
        bar_lambda_l = bar_lambda + eta_s\(l_Axk_old + Bk_hat*(bar_lambda-lambdak_old));
        bar_lambda_u = bar_lambda + eta_s\(u_Axk_old + Bk_hat*(bar_lambda-lambdak_old));
        bar_lambda_new   = zeros(length(lambdak_old), 1);
        bar_lambda_new(bar_lambda_l>0) = bar_lambda_l(bar_lambda_l>0);
        bar_lambda_new(bar_lambda_u>0) = bar_lambda_u(bar_lambda_u<0);

        increament_value = 0.5* sigma* eta_s* norm(bar_lambda_new - bar_lambda)^2;
        f_bar_value_new = f_bar_value(bar_lambda);
        if f_bar_value_new > f_bar_value_old + increament_value
            break
        else
            eta_s = eta_s* 0.2;
            s = s+1;
    ...
    f_bar_value_old = f_bar_value_new;
    bar_lambda      = bar_lambda_new;
end
end
%%
%%% End of time.m
