function [x, lambda, value, num_AS] =  DASA_SCREEN_6(A, y, l, u, lambda0, varargin)
...
%       OPTION.Newton_Method: 
%           whether use Newton method in line search
%           0 default, don't use Newton method
%           1 Newton method is used
%
% **< Author: USST, Yunlong Wang >**
% allow BB step size in SPARSA
% Version 4: add gradient info rmation: allow remove zeros in AP_+ and AP_-.
% Version 5.1: add sign information in SPARSA, based on AP_+ and AP_-.
%        (provide feasible points from SPARSA)
% Version 5.2: add Newton method. not completed 20220327
...
% if active_method > 0, active_epoch (defult 5) and gradient_info is better
% given
if isfield(OPTION, 'active_method')
    ...
    % whether do trucction in SPARSA with information of AS_p and AS_n
    % 0, dot not do trucntion
    % 1, default, do trunction in SPARSA
    if isfield(OPTION, 'Trunc_by_AS_pn')
        Trunc_by_AS_pn = OPTION.Trunc_by_AS_pn;
    else
        Trunc_by_AS_pn = 1;
    end
else
...
% whether us Newton method in line search
% 0, default, don't use newton method
% 1, use newton method in line search
if isfield(OPTION, 'Newton_method')
        Newton_method = OPTION.Newton_method;
    else
        Newton_method = 1;
end
...
Axk_old = A*xk_old;
h_l_k = max(0,l - Axk_old); h_u_k =min(0,u - Axk_old);
scaler = min( [l(h_l_k>0)./Axk_old(h_l_k>0); u(h_u_k<0)./Axk_old(h_u_k<0); 1] );
...
%         lambdak(AS_n) = min(lambdak(AS_n), 0);
        ...
        if k > 1
            ...
            [AS_p, AS_n] = update_AS_pn(AS_0, AS_p, AS_n); % update the sets AS_p and AS_n
        end
...
%         while 1  % nonmonotone linear search for sparsa
            ...
            if Trunc_by_AS_pn == 1 %% trunction in SPARSA with information of AS_p and AS_n
                if any(lambdak_l(AS_p) < 0) || any(lambdak_u(AS_n) > 0)
                lambdak(AS_p) = max(lambdak_l(AS_p),0);
                lambdak(AS_n) = min(lambdak_u(AS_n),0);
                end
            end
            %%%%%%%%%%%%%%%% compute search direction
            if screening ==1, start_working = 1; end
            ggk_old = 0.5*(h_l_old + h_u_old);
            if Newton_method == 1 &&  exist ('start_working', 'var') && r<3e-1 %&& size(A,1)<1000 %&& r<1e-1 %&& screening ==1 
                run_Newton_method = 1;
                ggk_old(AS_p) = h_l_old(AS_p);  ggk_old(AS_n) = h_u_old(AS_n);
                if  0 && Adersen_accelete == 1
                    [lambdak_ac,V] = A_A(lambdak_old2, lambdak, k, V, active_epoch, AS_0); % Adersen Acceleration
                    dk = lambdak_ac - lambdak_old;
                else
                xk = max(y + A'*lambdak, 0);
                h_l_k = l - A*xk;
                h_u_k = u - A*xk;
                BS1_p = intersect(AS_p, find((lambdak_l<r) & (alphak\h_l_k<-r)) ); % & (alphak*ggk_old<r)
                BS1_n = intersect(AS_n, find((lambdak_u>-r) & (alphak\h_u_k>r) ) );%  & (alphak*ggk_old>r)
                BS1 = union(BS1_p, BS1_n);
                BS_p = intersect(AS_p, find((lambdak_old<r/2) & (alphak\ggk_old<-r/2) ) ); 
                BS_n = intersect(AS_n, find((lambdak_old>-r/2) & (alphak\ggk_old>r/2) ) );
                if ~isempty(union(BS_p, BS_n))
                BS = union(BS_p, BS_n); % BS is the (active) set close to bound
                else
                BS = [];
                end
                FS = setdiff([1:length(lambdak)]', BS1); % FS is the free set 
                xk_old_zero_indic = (xk_old==0); 
                A_temp = A(:,~xk_old_zero_indic); y_temp = y(~xk_old_zero_indic);
                G_temp = A_temp*A_temp';
                G = ( G_temp + sqrt(r)*eye(size(A_temp,1)));
 
                % dk_newton = CG_method(G, ggk_old, ggk_old, min(r^2*1e-3,1e-7) ); dk = dk_newton;
                % right_term = l - A_temp*y_temp; right_term(AS_n) = u(AS_n) - A_temp(AS_n,:)*y_temp;
                % dk = G\right_term - lambdak_old; % G\right_term is optimal lambda
                dk_newton = G\ggk_old;  %dk = dk_newton;
                dk = lambdak - lambdak_old;
%                 dk(BS) = 0 - lambdak_old(BS); 
                end
            else
                run_Newton_method = 0;
                dk = lambdak - lambdak_old;
            ...
            s = 1;
            while 1 %%%%%%%%%%%%%%%% line search
                lambdak = Trunc(lambdak_old + s * dk, AS_p, AS_n);
                dvalue = D_funvalue(lambdak, A, y, l, u);
                if run_Newton_method == 1
                    increment = dk'*ggk_old/alphak ;%norm(dk)^2; % ; % norm(lambdak - lambdak_old)^2; % dk'*ggk_old;
                elseif run_Newton_method == 0
                    increment =  norm(lambdak - lambdak_old)^2; %dk'*ggk_old/alphak ;%
                end
                if dvalue >=  dvalue_avg + 0.5 * sigma * s * increment %nonmonotone linear search
                    Qk = xi*Qk + 1;
                    break
                end
                s = delta*s;
        ...
        while 1  % linear search for sparsa
            lambdak = Trunc(lambdak_old + alphak * dk, AS_p, AS_n);
            dvalue = D_funvalue(lambdak, A, y, l, u);
    ...
    xk = max(y + A'*lambdak, 0);
    Axk=A*xk;
    h_l_k = max(0,l - Axk); h_u_k =min(0,u - Axk);
    %=vvv================= update xk to feasible area
    scaler = min( [l(h_l_k>0)./Axk(h_l_k>0); u(h_u_k<0)./Axk(h_u_k<0); 1] );
    ...
    sigma_p_feasi_error =sigma_1*norm([h_l_k; h_u_k], 1);
    p_feasi_error = sqrt( norm(h_l_k)^2 + norm(h_u_k)^2 );
    ...
    if  active_method == 1
        if r < 0.01*median(u-l) && mod(k+1,active_epoch)==0
            ...
            if  gradient_info == 1
                zer_p = intersect(find(lambdak ==0 & h_l_k == 0),AS_p); % find the the zeros set belonging AS_p
                zer_n = intersect(find(lambdak ==0 & h_u_k == 0),AS_n); % find the the zeros set belonging AS_n
    ...
    if  k > 1
        ...
        fprintf('\n k=%2d p=%2.4e d=%2.4e alphak=%2.2e  bd_e=%0.3e  gap_error=%2.3e  #AS_0=%d  #AS_p=%d  #AS_n=%d [%d:%d:%d]',...
            k, pvalue, dvalue, s, p_feasi_error, gap_error, length(AS_0), length(AS_p), length(AS_n), hh, mm, ss)
        
%         diffgap = pvalue + sigma_p_feasi_error - pvalue_scaler
%         if abs(gap_error) + sigma_p_feasi_error < epsilon
         if gap_error < 1e-10 % epsilon
...
% lambdak_Trunc(lambda_nzero_indic) = lambdak;
...
%% Adersen Acceleration
function [lambdak_ac,V] = A_A(lambdak_old, lambdak, k, V,active_epoch, AS_0)
if size(V,2) == 0
    V = [lambdak_old,lambdak];
elseif mod(k,active_epoch) == 0
    unzero_lambdak = setdiff([1:size(V,1)], AS_0);
    V = [V(unzero_lambdak, max(end-active_epoch,1):end),lambdak];
% else
%     V = [V(:, max(end-active_epoch,1):end),lambdak];
end
if size(V,2) > active_epoch % mod(k+1,active_epoch)==0
    U = diff(V,1,2); Q = U'*U ; Q = Q + 1e-6*eye(size(U,2));
    w = Q\ones(size(Q,1),1); %CG_method( Q, ones(size(Q,1)), ones(size(Q,1)) );
    w = w./sum(w);
    lambdak_ac = V(:,2:end)*w;
else
    lambdak_ac = lambdak;
end
%%
