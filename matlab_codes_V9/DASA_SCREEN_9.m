        ...
        if (r < 0.1*median(u-l) || 0) && (mod(k+1,active_epoch)==0)
            ...
            if  gradient_info > 0  % gradient info is contained in h_l and h_u
                ...
                if any(zero_in_p) || any(zero_in_n) || any(zero_in_pn)
                    fprintf('\n length(zer_p):  %d    length(zer_n):  %d    length(zer_pn):  %d', [sum(zero_in_p), sum(zero_in_n), sum(zero_in_pn)])
                end
%                 indi_in = (indi_in | zer_pn);    % update the zero set (add zer_pn to AS_0)
            ...
            if any(zero_in_pn)
                ...
                AS_quasi_0 = AS_free(zero_pn);  % for full length lambda
            end
            num_AS_0(k) = sum(AS_0);
            num_AS_p(k) = sum(AS_p);
            num_AS_n(k) = sum(AS_n);
