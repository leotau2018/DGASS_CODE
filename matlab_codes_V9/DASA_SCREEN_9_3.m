function [x, lambda, value, num_AS, k, time1, AS_0] =  DASA_SCREEN_9_3(A, y, l, u, lambda0, varargin)
...
if nargin < 5
    if ~exist('A', 'var')
        warning("please give matrix 'A'")
    end
    if ~exist('y', 'var')
        warning("please give vector 'y'")
    end
    if ~exist('l', 'var')
        warning("please give vector 'l'")
    end
    if ~exist('u', 'var')
        warning("please give vector 'u'")
    end
    if ~exist('lambda0', 'var')
        warning("'lambda0' is not given and randomly generated")
        lambda0 = zeros(length(l), 1);
    end
end
if isempty(varargin)
    OPTION = [];
...
while k <= maxiter
    OPTION.AS_quasi_0 = AS_quasi_0;
%     SCR_S = AS_0| AS_quasi_0;
    %======lambdak_raw(AS_0) = 0; %screening the components to be zeros
    if (k>1) && ((screening == 1) || (quasi_screening == 1) )% implement screening

        SCR_S =  AS_0| AS_quasi_0;
        lambdak_raw(AS_0) = 0; % adjust the elements of lambda / remove the zeros entries
        lambdak_raw(AS_quasi_0) = 0; % adjust the elements of lambda / remove the zeros entries
%         lambdak_raw(AS_quasi_0) = 0; % AS_quasi_0 is not in AS_0
        A = A_raw(~SCR_S,:);
        l = l_raw(~SCR_S); u = u_raw(~SCR_S);
        lambdak = lambdak_raw(~SCR_S);
        lambdak_old  = lambdak_old_raw(~SCR_S);
        xk = max(y + A'*lambdak, 0);
        if quasi_screening == 1
            lambdak_real= lambdak_raw(~AS_0) ;
        else
            lambdak_real = lambdak;
        end
    elseif k>1
        SCR_S =  AS_0| AS_quasi_0; 
        A = A_raw(~SCR_S,:);
    ...
    if k >1
        ...
        lambdak_old  = lambdak_raw(~SCR_S); % lambdak_old_raw(~SCR_S); it is equivalent
        xk_old = max(y + A'*lambdak_old, 0);
        ...
        while 1  % nonmonotone linear search for sparsa / proximal gradient algorithm (PGA)
            ...
            if (Newton > 0) % && (resi<1e-1) %&& (screen_open == 1)                
                ...
                OPTION.newton_tol = min(1e-4, 0.5*resi);
                ...
                lambdak_l = lambdak_old + l_Axk_old/alphak;
                lambdak_u = lambdak_old + u_Axk_old/alphak;
%                 if (gradient_info > 0) && (quasi_screening == 1)
%                     if any(ZS_n) || any(ZS_p)
%                         lambdak_l(ZS_n ) = 0;
%                         lambdak_u(ZS_p) = 0;
%                     end
%                     if any(ZS_quasi_0)
%                         lambdak_l(ZS_quasi_0) = 0;
%                         lambdak_u(ZS_quasi_0) = 0;
%                     end
%                 end
                %%%%% implement proximal operation
                lambdak = zeros(size(lambdak_old));
    ...
    lambdak_raw(~SCR_S) = lambdak; % update the accepted lambdak
    if quasi_screening == 1
        lambdak_real(~ZS_quasi_0) = lambdak;
    else
        lambdak_real = lambdak;
    end
...
%     Sigma_p_feasi_error(k) = sigma_p_feasi_error;
    ...
    if  active_method == 1
        if (r < 0.1*median(u-l) || 0) && (mod(k+1,active_epoch)==0)
            ...
            T_p = Axk + A_rownorm * r;
            T_n = Axk - A_rownorm * r;
            indi_l = (T_p < u_raw(~AS_0));
            indi_u = (T_n > l_raw(~AS_0));
            ...
            if  gradient_info > 0  % gradient info is contained in h_l and h_u
                if gradient_info == 1 % use exact gradient information
                    zero_in_p = (lambdak_real ==0 & l_raw(~AS_0)-Axk <= 0) & indi_ll; % find the the zeros set belonging AS_p
                    zero_in_n = (lambdak_real ==0 & u_raw(~AS_0)-Axk >= 0) & indi_uu; % find the the zeros set belonging AS_n
                elseif gradient_info == 2 % use enhance gradient information (not exactly)
                    zero_in_p = (lambdak_real < 1e-6) & (lambdak_real + (l_Axk)/alphakB < -1e-9) & indi_ll; % find the the zeros set belonging AS_p
                    zero_in_n = (lambdak_real > 1e-6) & (lambdak_real + (u_Axk)/alphakB > 1e-9) & indi_uu; % find the the zeros set belonging AS_n
                end
...
%                 indi_uu  = (indi_uu & ~zero_in_n); % update the AS_n (remove zer_n from AS_n)
            ...
            if any(zero_in_pn)
                AS_quasi_0 = false(length(lambdak_raw), 1);
                AS_quasi_0(AS_free(zero_in_pn)) = 1;  % for full length lambda
