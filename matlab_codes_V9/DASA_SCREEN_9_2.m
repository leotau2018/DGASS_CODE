function [x, lambda, value, num_AS, k, time1, AS_0] =  DASA_SCREEN_9(A, y, l, u, lambda0, varargin)
...
dvalue_old = D_funvalue(lambdak_old, xk_old, y, l, u);
%===================
x_plus_indic = [1:n];
...
while k <= maxiter
    ...
    if screening == 1 % implement screening
        lambdak_raw(AS_0) = 0;       % adjust the elements of lambda / to remove these zeros entries
        A = A_raw(~AS_0,:);
    ...
    if k >1
        ...
        xk = max(y+A'*lambdak);
        dvalue_old = D_funvalue(lambdak_old, xk, y, l, u);
...
%             end
            ...
            if (sparsa_line_search == 1)
                xk = sparse(max(y + A'*lambdak, 0));
                dvalue = D_funvalue(lambdak, xk, y, l, u);
...
%                     lambdak_n = Trunc(lambdak_n, AS_p(~AS_0), AS_n(~AS_0)) ;
                    xk = max(y + A'*lambdak, 0);
                    dvalue = D_funvalue(lambdak_n, xk, y, l, u);
    ...
    J_x_indi = (xk >0);
    A_active = A(:,J_x_indi);
    xk_active = xk(J_x_indi);
    Axk = A_active*xk_active;
    l_Axk = l - Axk; 
    u_Axk = u - Axk;
    ...
    xk_scaler_active = xk_active*scaler;
    pvalue_scaler = 0.5*(norm(xk_scaler)^2 + norm(y)^2 - 2*xk_active'*y(J_x_indi));
    Axk=A_active*xk_scaler_active;
    %===================
    ...
    dvalue = D_funvalue(lambdak, xk_active, y, l, u);
    ...
    if  active_method == 1
        if (r < 0.1*median(u-l) || 0) && (mod(k+1,active_epoch)==0)
            ...
            if  gradient_info > 0  % gradient info is contained in h_l and h_u
                ...
                zero_in_p = (lambdak ==0 & l_Axk <= 0) & indi_ll; % find the the quasi-zeros set belonging AS_p
                zero_in_n = (lambdak ==0 & u_Axk >= 0) & indi_uu; % find the the quasi-zeros set belonging AS_n
                elseif gradient_info == 2 % use enhance gradient information (not exactly)
                    zero_in_p = (lambdak < 1e-6) & (lambdak + (l_Axk)/alphakB < -1e-9) & indi_ll; % find the the quasi-zeros set belonging AS_p
                    zero_in_n = (lambdak > 1e-6) & (lambdak + (u_Axk)/alphakB >  1e-9) & indi_uu;  % find the the quasi-zeros set belonging AS_n
                end
                ...
                if any(zero_in_p) || any(zero_in_n) || any(zero_in_pn)
                    fprintf('\n length(zero_p):  %d    length(zero_n):  %d    length(zero_pn):  %d', [sum(zero_in_p), sum(zero_in_n), sum(zero_in_pn)])
                end
%                 indi_in = (indi_in | zero_pn);    % update the zero set (add zer_pn to AS_0)， ATTENTION: NOT SAFE TO DELATE
            ...
            if any(zero_in_pn)
                AS_quasi_0 = zero_in_pn;
                AS_quasi_0(indi_in) = [];       % logical for next new lambda, same length as next new lambda after screen
%                 ZS_quasi_0 =  AS_free(zero_pn);
            end
            num_AS_0(k) = sum(AS_0);
            num_AS_p(k) = sum(AS_p);
            num_AS_n(k) = sum(AS_n);
...
%% ================================= sub function ==================================
%% Dual function value
function dvalue = D_funvalue(lambdak, xk, y, l, u)
