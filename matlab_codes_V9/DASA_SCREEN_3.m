function [x, lambda, value, num_AS, flag] =  DASA_SCREEN_3(A, y, l, u, lambda0, varargin)
...
while k <= maxiter
    ...
    if k >1
        ...
        while 1  % nonmonotone linear search
            ...
            if alphak>1e+10
                fprintf('\n === too small step size %.3e ====  r=%.3e; gap=%.4e ',alphak, r,  gap_error);
                break;
            end
        end
    ...
    % only when the detection is safe(r is sufficient small -> the ragion is sufficient narrow)
    if  active_method == 1
        if r < 0.05*median(u-l) && mod(k+1,active_epoch)==0
            fprintf('=>screening:r=%.2e',r)
            A_rownorm = sqrt(sum(A.*A,2));
    ...
    if  k > 1
        ...
        fprintf('\n k=%2d p=%2.4e d=%2.4e alphak=%2.2e  bd_e=%0.3e  gap_error=%2.3e  #AS_0=%d  #AS_p=%d  #AS_n=%d [%d:%d:%d]',...
            k, pvalue, dvalue, alphak, p_feasi_error, gap_error, sum(AS_0), length(AS_p), length(AS_n), hh, mm, ss)
...
%         if abs(gap_error) + sigma_p_feasi_error < epsilon
         if gap_error < epsilon
            if pvalue_scaler >= dvalue 
                convergence = 1; flag=0;
                fprintf('\n === convergence====  r=%.3e; gap=%.4e ', r,  gap_error);
%                 fprintf('\n === convergence====  relative_error=%.3e; \n', relative_error);
                break;
            else
                convergence = 0; flag=1;
                fprintf(' ==> OVERFLOW')
%                 continue
            end
         else
             if time1>=600
                convergence = 0; flag=2;
                fprintf('\n === break because time %ds ====  r=%.3e; gap=%.4e ',600, r,  gap_error);
                break;  
             elseif alphak>1e+10
                flag = -1;
                fprintf('\n === to0 small step size %.3e ====  r=%.3e; gap=%.4e ',alphak, r,  gap_error);
                break;
             end
         end
    else
...
xk = max(y + A'*lambdak, 0);
f   = 0.5*(y'*y - xk'*xk);
psi_l = l(lambdak>0)'*lambdak(lambdak>0);
psi_u = u(lambdak<0)'*lambdak(lambdak<0);
psi = sum([psi_l,psi_u]);
dvalue = f + psi;
