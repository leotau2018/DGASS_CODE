function [x, lambda, value, num_AS] =  DASA_SCREEN_1(A, y, l, u, lambda0, varargin)
...
while k <= maxiter
    ...
    r = sqrt( 2*(pvalue + sigma_bound_error - dvalue_max)/gamma1 );
    % only when the detection is safe(r is sufficient small -> the ragion is sufficient narrow)
    if  active_method == 1
        if r < 1e-1 && mod(k+1,active_epoch)==0
    ...
    if  k > 1
        ...
        fprintf('\n k=%2d p=%2.4e d=%2.4e alphak=%2.2e  bd_e=%0.3e  gapupper=%2.3e  #AS_0=%d  #AS_p=%d  #AS_n=%d [%d:%d:%d]',...
            k, pvalue, dvalue, alphak, bound_error, pvalue + sigma_bound_error - dvalue, sum(AS_0), length(AS_p), length(AS_n), hh, mm, ss)
        if abs(gap_error) + sigma_bound_error < epsilon
            if pvalue >= dvalue || 1
                convergence = 1;
                fprintf('\n === convergence====  r=%.3e; gapceil=%.4e ', r, pvalue + sigma_bound_error - dvalue);
